...
import 'package:circlemenu/geometry.dart';
import 'package:circlemenu/radial_menu_collisions.dart';
...
class CollidingRadialMenu extends StatefulWidget {
  ...
  @override
  _CollidingRadialMenuState createState() => _CollidingRadialMenuState();
}
...
class _CollidingRadialMenuState extends State<CollidingRadialMenu> {
  ...
  void _findNonCollidingArc(BoxConstraints constraints) {
    ...
    final centerOfScreen =
        new Point(constraints.maxWidth / 2, constraints.maxHeight / 2);

    Set<Point> intersections =
        intersect(screenSize, origin, widget.radius + (50.0 / 2));
    ...
    if (intersections.length == 2) {
      print(
          'Intersection points: ${intersections.first}, ${intersections.last}');

      radialArc = _createStartAndEndAnglesFromTwoPoints(
          intersections, origin, centerOfScreen);
  ...
  List<Widget> _buildDebugPoints(BoxConstraints constraints, Offset anchor) {
    ...
    if (radialArc.sweepAngle() != Angle.fullCircle) {
      ...
      List<Widget> dots = []
        ..add(_createDot(startPoint))
        ..add(_createDot(endPoint));
  ...
  Arc _createStartAndEndAnglesFromTwoPoints(
      Set<Point> menuEdgePoints, Point origin, Point centerOfScreen) {
    ...
    final directionToCenter = new Angle.fromRadians(
        new PolarCoord.fromPoints(origin, centerOfScreen).angle);
    bool isOnLeftSideOfScreen = origin.x < centerOfScreen.x;
    final isClockwise = isOnLeftSideOfScreen;
    print('isClockwise: $isClockwise');

    Angle angle1 = new Angle.fromRadians(
        new PolarCoord.fromPoints(origin, menuEdgePoints.first).angle);
    ...
    Angle angle2 = new Angle.fromRadians(
        new PolarCoord.fromPoints(origin, menuEdgePoints.last).angle);
    Arc arc2 = new Arc(
      from: angle2,
      to: directionToCenter,
      direction: isClockwise
    ...
    if (isClockwise) {
      ...
      if (arc1.sweepAngle().toRadians().abs() <
          arc2.sweepAngle().toRadians().abs()) {
    ...
    Angle angleToCenterOfScreen = new Angle.fromRadians(
        new PolarCoord.fromPoints(origin, centerOfScreen).angle);

    final Angle centerToIntersectDelta =
        angleToCenterOfScreen - intersectionAngle;
    print('angleToCenterOfScreen: $angleToCenterOfScreen, intersectionAngle');
    print('centerToIntersectDelta: $centerToIntersectDelta');

    if (!isClockwise) {
      startAngle =
          new Angle.fromRadians(startAngle.toRadians(forcePositive: true));
  ...
  Set<Point> _reduceIntersections(
      Set<Point> intersections, Point origin, Point centerOfScreen) {
    Set<Point> twoPoints = new Set();

    final Angle directionToCenter = new Angle.fromRadians(
        new PolarCoord.fromPoints(origin, centerOfScreen).angle);
    ...
    for (Point intersection in intersections) {
      Angle intersectionAngle = new Angle.fromRadians(
          new PolarCoord.fromPoints(origin, intersection).angle);
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        List<Widget> dots = widget.debugMode
            ? _buildDebugPoints(constraints, widget.anchor)
            : [];
...
class _RadialMenuState extends State<RadialMenu>
  ...
  static const Color expandedBubbleColor = Color(0xff666666);
    ...
    switch (_menuController.state) {
      ...
      case RadialMenuState.opening:
        ...
        if (0.0 < _menuController.progress && _menuController.progress < 0.5) {
          rotation = lerpDouble(0.0, pi / 4,
              Interval(0.0, 0.5).transform(_menuController.progress));
        } else {
          rotation = lerpDouble(pi / 4, 0.0,
  ...
  Widget buildActivationRibbon() {
    ...
    MenuItem activeItem = widget.menu.items.firstWhere(
        (MenuItem item) => item.id == _menuController._activationId);
    ...
    if (_menuController.state == RadialMenuState.activating) {
      ...
      final double indexDivsor = menuSweepAngle == Angle.fullCircle
          ? itemCount.toDouble()
          : (itemCount - 1).toDouble();
      final Angle initialItemAngle = widget.arc.from +
          (menuSweepAngle * (activeIndex.toDouble() / indexDivsor));
...
class RadialMenuController extends ChangeNotifier {
  ...
  void activity(String menuItemId) {
    if (_state == RadialMenuState.expanded) {
